...
vi.mock('@iconify/react', () => ({
  Icon: ({ icon, color }: { icon: string; color?: string }) => (
    <span aria-hidden="true" data-color={color} data-icon={icon} />
  ),
}));
...
it('uses custom presentation overrides and falls back for a blank tooltip', () => {
  const { rerender } = render(
    <AIAssistantToggle
      {...closedToggleArgs}
      tooltipTitle="Cluster assistant"
      icon="mdi:robot"
      iconColor="white"
    />
  );
  expect(screen.getByRole('button', { name: 'Cluster assistant' })).toBeTruthy();
  expect(document.querySelector('[data-icon="mdi:robot"]')?.getAttribute('data-color')).toBe(
    'white'
  );
