...
describe('sanitizeContent', () => {
  ...
  it('removes malformed script markup and decodes entities only once', () => {
    expect(sanitizeContent('safe<script>alert(1)</script>end')).toBe('safealert(1)end');
    expect(sanitizeContent('safe<script')).toBe('safe');
