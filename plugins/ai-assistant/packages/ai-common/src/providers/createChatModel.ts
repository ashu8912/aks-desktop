    ...
    switch (providerId) {
      ...
      case 'azure':
        ...
        return new AzureChatOpenAI({
          azureOpenAIEndpoint: normalizeAzureEndpoint(c.endpoint),
      ...
      case 'vllm': {
        ...
        let url = trimTrailingSlashes(c.baseUrl);
...
function normalizeAzureEndpoint(endpoint: string): string {
  try {
    ...
    const withoutTrailingSlashes = trimTrailingSlashes(endpoint);
    const openAIPath = withoutTrailingSlashes.toLowerCase().indexOf('/openai/');
    return openAIPath >= 0 ? withoutTrailingSlashes.slice(0, openAIPath) : withoutTrailingSlashes;
  }
}
